...
export default function JournalPage() {
  ...
  const columns = [
    { key: 'je_number', label: 'رقم القيد', render: je => <span className="font-mono text-primary-600 font-semibold text-xs">{je.je_number || je.serial}</span> },
  ...
  return (
    <div className="page-enter space-y-5">
      <PageHeader
        ...
        actions={<button onClick={() => setShowAdd(true)} className="btn-primary">+ قيد جديد</button>}
      />
      <div className="card flex gap-3 flex-wrap">
        ...
        <button onClick={() => { setFilters({ status: '', date_from: '', date_to: '' }); load() }} className="btn-ghost">مسح</button>
      </div>
      <div className="card p-0 overflow-hidden">
        <DataTable columns={columns} data={jes} loading={loading} onRowClick={setViewJE} />
      </div>
      <AddJEModal open={showAdd} onClose={() => setShowAdd(false)} accounts={accounts}
        onSaved={() => { load(); setShowAdd(false) }} />
      {viewJE && <JEDetailModal je={viewJE} onClose={() => setViewJE(null)}
...
function AddJEModal({ open, onClose, accounts, onSaved }) {
  ...
  const [form,  setForm]  = useState({ description: '', entry_date: new Date().toISOString().split('T')[0], reference: '', je_type: 'GJE' })
  ...
  const handleSave = async (andPost = false) => {
    ...
    try {
      const je = await api.accounting.createJE({
        ...form,
      ...
      if (andPost) await api.accounting.postJE(je.data?.id || je.id)
      toast(`تم ${andPost ? 'ترحيل' : 'حفظ'} القيد`, 'success')
      onSaved()
    } catch (e) {
      setError(e.message)
    } finally {
...
function JEDetailModal({ je, onClose, onPosted }) {
  ...
  return (
    <Modal open={!!je} onClose={onClose} title={`تفاصيل القيد — ${je.je_number || je.serial}`} size="lg">
      <div className="space-y-4">
