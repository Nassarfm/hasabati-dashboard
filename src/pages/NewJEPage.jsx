/* hasabati-new-je-v5 — Enterprise Journal Entry */
  ...
  return (
    <div ref={containerRef} className="relative w-full">
      ...
      {open && filtered.length > 0 && (
        <ul ref={listRef}
          className="absolute z-[300] top-full right-0 left-0 bg-white border border-slate-200 rounded-xl shadow-2xl max-h-64 overflow-y-auto mt-0.5 list-none p-0 m-0">
          {filtered.map((a, i) => {
...
// ══════════════════════════════════════════════════════════
// EXCEL IMPORT MODAL — xlsx حقيقي بدلاً من CSV
// ══════════════════════════════════════════════════════════
  ...
  const handleFile = async (e) => {
    ...
    try {
      ...
      for (let i = 0; i < dataRows.length; i++) {
        ...
        const debit  = parseFloat(r[2]) || 0
        const credit = parseFloat(r[3]) || 0
        const cc     = String(r[4] || '').trim()
        const br     = String(r[5] || '').trim()
        const proj   = String(r[6] || '').trim()
        ...
        parsed.push({
          id: Math.random(),
          account_code: code, account_name: acct?.name_ar || '', account: acct || null,
      ...
      if (Math.abs(totalDR - totalCR) > 0.01)
        errs.push({ row: 0, msg: `القيد غير متوازن — مدين ${fmt(totalDR,2)} ≠ دائن ${fmt(totalCR,2)}` })

      setRows(parsed)
      setErrors(errs)
    } catch {
      toast('خطأ في قراءة الملف — تأكد أنه Excel صحيح', 'error')
    } finally {
      setLoading(false)
      e.target.value = ''
    }
    ...
    <div className="fixed inset-0 z-[200] flex items-center justify-center">
      ...
      <div className="relative bg-white rounded-2xl shadow-2xl w-[720px] max-h-[80vh] flex flex-col">
        ...
        <div className="p-6 flex-1 overflow-y-auto space-y-4">
          <div className="flex gap-3 items-center">
            <button onClick={downloadTemplate}
              className="flex items-center gap-2 px-4 py-2 rounded-xl border border-emerald-300 text-emerald-700 text-sm font-medium hover:bg-emerald-50">
              ⬇️ تحميل قالب Excel
            </button>
          ...
          {rows.length === 0 && (
            <div className="bg-slate-50 rounded-xl p-4 text-xs text-slate-500 space-y-1">
              ...
              <div>• العمود الأول: كود الحساب (مطابق تماماً لدليل الحسابات)</div>
              <div>• العمود الثاني: بيان السطر</div>
              <div>• العمود الثالث: قيمة المدين (أو 0)</div>
              <div>• العمود الرابع: قيمة الدائن (أو 0)</div>
          ...
          {errors.length > 0 && (
            <div className="bg-red-50 border border-red-200 rounded-xl p-3 space-y-1">
              ...
              {errors.map((e, i) => (
                <div key={i} className="text-xs text-red-600">{e.row > 0 ? `سطر ${e.row}:` : ''} {e.msg}</div>
              ))}
            </div>
          )}
          ...
          {rows.length > 0 && (
            <div className="rounded-xl border border-slate-200 overflow-hidden">
              ...
              <div className="grid grid-cols-6 bg-slate-100 text-xs font-semibold text-slate-600">
                {['الحساب', 'البيان', 'مدين', 'دائن', 'CC', 'الفرع'].map(h => (
        ...
        <div className="flex justify-end gap-3 px-6 py-4 border-t bg-slate-50 rounded-b-2xl">
          ...
          <button
            disabled={rows.length === 0 || hasBlockingError}
...
// ══════════════════════════════════════════════════════════
// AI NARRATIVE PANEL — يستدعي Backend فقط (لا Anthropic مباشرة)
// ══════════════════════════════════════════════════════════
      ...
      const payload = {
        ...
        lines: validLines.map(l => {
          ...
          return {
            ...
            account_name: l.account_name || acct?.name_ar || '',
  ...
  return (
    <div className="card space-y-3">
      ...
      <textarea className="input w-full text-sm" rows={2}
        placeholder="ملاحظات إضافية للمحاسب (اختياري)..."
            ...
            <div className="bg-amber-50 border border-amber-200 rounded-xl p-3">
              ...
              <ul className="space-y-1">
                {result.risks.map((r, i) => (
                  <li key={i} className="text-xs text-amber-700 flex gap-2"><span className="shrink-0">•</span><span>{r}</span></li>
                ))}
              </ul>
            </div>
          )}
...
export default function NewJEPage({
  ...
  // ── Keyboard Shortcuts — useCallback يمنع stale closures ──
  useEffect(() => {
    const handleDown = (e) => {
  ...
  const PeriodBadge = () => {
    ...
    if (periodState.status === 'open')      return <span className="text-xs text-emerald-700 bg-emerald-50 px-3 py-1.5 rounded-full border border-emerald-200 font-medium">✅ {periodState.periodName} — مفتوحة</span>
    if (periodState.status === 'closed')    return <span className="text-xs text-red-700 bg-red-50 px-3 py-1.5 rounded-full border border-red-200 font-medium">🔒 {periodState.periodName} — مغلقة</span>
    if (periodState.status === 'not_found') return <span className="text-xs text-amber-700 bg-amber-50 px-3 py-1.5 rounded-full border border-amber-200 font-medium">⚠️ لا توجد سنة مالية</span>
    return <span className="text-xs text-orange-700 bg-orange-50 px-3 py-1.5 rounded-full border border-orange-200 font-medium">⚠️ تعذّر التحقق</span>
  }

  return (
    <div className="page-enter space-y-4">

      {/* ── Breadcrumb — قابل للنقر ── */}
      ...
      <div className="flex items-center justify-between">
        ...
        <div className="flex items-center gap-2">
          ...
          <button onClick={() => setShowHotkeys(true)} className="px-3 py-2 rounded-xl border border-slate-200 text-slate-500 text-xs hover:bg-slate-50" title="اختصارات (Alt)">⌨️</button>
          ...
          <button onClick={handleDuplicate} disabled={!isFormOpen}
            className="flex items-center gap-1.5 px-3 py-2 rounded-xl border border-slate-200 text-slate-600 text-xs hover:bg-slate-50 disabled:opacity-40" title="نسخ القيد (Alt+D)">
            📋 نسخ
          </button>
        </div>
      </div>
        ...
        <div className="grid grid-cols-12 gap-4">
          ...
          <div className="col-span-2">
            <Field label="المرجع">
              ...
              {/* Reference Warning — كامل مع رقم القيد والتاريخ وزر العرض */}
              {refWarning && (
                <div className="mt-1.5 p-2.5 bg-amber-50 border border-amber-300 rounded-xl text-xs text-amber-700 space-y-1.5">
                  ...
                  <div className="flex items-center justify-between">
                    ...
                    <button
                      onClick={() => toast(`القيد: ${refWarning.serial} | التاريخ: ${refWarning.entry_date} | الحالة: ${refWarning.status}`, 'info')}
                      className="text-xs px-2 py-0.5 rounded-lg bg-amber-200 hover:bg-amber-300 text-amber-800 font-medium">
                      👁️ عرض
                    </button>
                  </div>
                    ...
                    <div className="grid items-center"
                      ...
                      <div className="px-1 py-1.5">
                        <input className="input text-xs w-full" placeholder="بيان السطر"
                          value={line.description} onChange={e => setLine(line.id, { description: e.target.value })} />
                      </div>

                      <div className="px-1 py-2 text-center">
                        {acct
                          ? <span className={`text-xs px-1.5 py-0.5 rounded-full ${tc.color || 'bg-slate-100 text-slate-600'}`}>{tc.label || acct.account_type}</span>
                      ...
                      {/* Branch — disabled إذا الحساب لا يحتاج أبعاد */}
                      <div className="px-1 py-1.5">
                        <select
                          className={`select text-xs w-full ${needsDim && acct?.dim_branch_required && !line.branch_code ? 'border-amber-400 bg-amber-50' : ''} ${!needsDim ? 'opacity-30 cursor-not-allowed' : ''}`}
                          disabled={!needsDim}
                      ...
                      {/* CC — disabled إذا الحساب لا يحتاج أبعاد */}
                      <div className="px-1 py-1.5">
                        <select
                          className={`select text-xs w-full ${needsDim && acct?.dim_cc_required && !line.cost_center ? 'border-amber-400 bg-amber-50' : ''} ${!needsDim ? 'opacity-30 cursor-not-allowed' : ''}`}
                          disabled={!needsDim}
                      ...
                      {/* Expense Class — يظهر فقط لحسابات المصروفات التي تحتاج أبعاد */}
                      <div className="px-1 py-1.5">
                        {isExpense && needsDim ? (
                          <select
                            className={`select text-xs w-full ${needsDim && acct?.dim_exp_class_required && !line.expense_classification_code ? 'border-amber-400 bg-amber-50' : ''}`}
              ...
              <div className="col-span-4 px-2 py-3 flex items-center gap-3">
                {balanced
                  ...
                  : <><span className="text-red-500 text-sm">⚠️ فرق: {fmt(Math.abs(totalD-totalC),2)}</span>
                     <button onClick={autoBalance} className="text-xs px-2 py-0.5 rounded-lg bg-primary-100 text-primary-700 hover:bg-primary-200" title="Alt+B">⚡ توازن تلقائي</button></>}
              </div>
              <div />
            </div>
          </div>
      ...
      <div className="flex items-center justify-between bg-white border border-slate-200 rounded-xl px-6 py-4 sticky bottom-4 shadow-lg">
        ...
        <div className="flex gap-3">
          {isFormOpen && !balanced && (
            <button onClick={autoBalance} className="px-4 py-2.5 rounded-xl text-sm font-medium border border-primary-300 text-primary-700 hover:bg-primary-50">
              ⚡ توازن تلقائي
            </button>
          )}
